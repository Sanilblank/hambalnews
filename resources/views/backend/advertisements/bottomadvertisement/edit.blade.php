        ...
        <div class="row mt-3">
            <div class="col-md-12">
                <div class="card">
                    ...
                    <div class="card-body">
                        <form action="{{route('updatebottom', $bottom_advertisement->id)}}" method="POST" enctype="multipart/form-data">
                            @csrf
                            @method("PUT")

                            <div class="form-group">
                                <label for="Link">Link / URL (URL to the advertisement website/ FB page, stc.)</label>
                                <input type="text" name="link" class="form-control {{($errors->any() && $errors->first('link')) ? 'is-invalid' : ''}}" value="{{$bottom_advertisement->link}}">
                                @if($errors->any())
                            ...
                            <div class="form-group">
                                <input type="checkbox" name="status" value="1"{{$bottom_advertisement->status == 1? "checked" : ""}}>
